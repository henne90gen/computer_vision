...
// Use the polynomial y(x) to map values x of the input image to values y of an output image by creating and using a
    ...
    case cv::EVENT_LBUTTONDOWN: {
        ...
        if (nb_points < MaxPoints) {
            ...
            if (nb_points) { // second point in SrcPtInt
                ...
                cv::Mat lookupTable = createLookupTable();

                cv::Point lastPoint(-1, -1);
                for (int x = 0; x < MapCurveimage512.cols; x++) {
                    int lookupX = (int)((float)x / (float)MapCurveimage512.cols * 256.0f);
                    int y = 512 - lookupTable.data[lookupX] * 2;

                    if (lastPoint.x != -1 && lastPoint.y != -1) {
                        line(MapCurveimage512, lastPoint, cv::Point(x, y), CV_RGB(255, 255, 255));
                    }
                    lastPoint.x = x;
                    lastPoint.y = y;
                }
